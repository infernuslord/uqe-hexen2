...
//#define VERSION_TEXT "1.18" // jkrige
...
int main(int argc, char **argv)
{
	...
	// jkrige
	//logprint("GENMODEL Version "VERSION_TEXT" ("__DATE__")\n\n");
	// jkrige

	// jkrige
	SetConsoleTitle("UQE Hexen II GENMODEL");

	init_log("uqehx2mdl.log");
	logprint("UQEHX2MDL 1.16 -- based on GenModel by John Carmack\n");
    logprint("===================================================\n");
	...
	if (argc ==1)
	{
		//logprint("usage: genmodel [-archive path] [-opt] model.hc\n");
		logprint("usage: uqehx2mdl [-archive path] [-opt] model.hc\n");
		return 0;
	}
	path[0] = 0;
	...
	if (i > 4 && strcmpi(&path[i-4],".mdl") == 0)
	{
		...
		{
			logerror ("Could not rename file!\n"); // jkrige - was Error()
		}
	}
	else
	{
...
void WriteFrame (FILE *modelouthandle, int framenum)
{
	...
	// all of these are byte values, so no need to deal with endianness
		...
		if (tarray[j].lightnormalindex > NUMVERTEXNORMALS)
			logerror ("invalid lightnormalindex %d\n",
					tarray[j].lightnormalindex); // jkrige - was Error()
...
// Calculate the bounding box for this model
	if (!ModelReadIn)
	{
		for (i=0 ; i<3 ; i++)
		{
			logprint ("framesmins[%d]: %f, framesmaxs[%d]: %f\n",
...
//
	if (!framecount)
	{
		logprint ("no frames grabbed, no file generated\n");
		return;
	}
	
	if (!skincount)
		logerror ("frames with no skins\n"); // jkrige - was Error()
	...
	logprint ("%4d frame(s)\n", model.numframes);
	logprint ("%4d ungrouped frame(s), including group headers\n", framecount);
	logprint ("%4d skin(s)\n", model.numskins);
	logprint ("%4d degenerate triangles(s) removed\n", degeneratetris);
	logprint ("%4d triangles emitted\n", model.numtris - degeneratetris);
	logprint ("pixels per triangle %f\n", averagesize);
	logprint ("%4d numverts\n", model.numverts);

	if (ModelReadIn)
	{
		logprint ("file size: %d (Saved %d)\n", (int)ftell (modelouthandle), original_size - ftell(modelouthandle) );
	}
	else
	{
		logprint ("file size: %d\n", (int)ftell (modelouthandle) );
	}
	logprint ("---------------------\n");
...
void ReadModel(char *FileName)
{
	...
	if (!FH)
	{
		logerror ("Could not open model %s\n",FileName); // jkrige - was Error()
	}
	...
	{
		logerror ("Invalid model version for file %s\n",FileName); // jkrige - was Error()
	}
	...
	for (i=0 ; i<model.numframes ; i++)
	{
		...
		{
			logerror ("group frames not implemented"); // jkrige - was Error()
...
// dword alignment for each scan
	...
	logprint ("skin width: %i (unpadded width %i)  skin height: %i\n",
...
//
	...
	logprint("Number of triangles (including degenerate triangles): %d\n",
			model.numtris);
...
//
	...
	logprint("Number of vertices: %i\n", model.numverts);

	logprint("Extracted scaling info: width %d, height %d\n",
...
void Cmd_Skin (void)
{
	...
	time1 = FileTime (file1);
	if (time1 == -1)
		logerror ("%s not found", file1); // jkrige - was Error()
	...
	if (TokenAvailable ())
	{
		...
		if (skins[skincount].interval <= 0.0)
			logerror ("Non-positive interval"); // jkrige - was Error()
	}
	else
	{
	...
	if (skincount > MAXSKINS)
		logerror ("Too many skins; increase MAXSKINS"); // jkrige - was Error()
}
...
static int ExtractDigit(byte *pic, int x, int y)
{
	...
	*buffer = '\0';
	...
	logerror ("Unable to extract scaling info from skin PCX."); // jkrige - was Error()
}
...
//
	...
	{
		logerror ("Number of triangles doesn't match\n"
			"Base frame: %d, frame %s: %d\n",
			model.numtris, frame, numtris); // jkrige - was Error()
	}
...
// set the intervals
	if (isgroup && TokenAvailable ())
	{
		...
		if (frames[framecount].interval <= 0.0)
			logerror ("Non-positive interval %s %f", token,
					frames[framecount].interval); // jkrige - was Error()
	}
	else
	{
		frames[framecount].interval = 0.1F;
...
//
	for (i=0 ; i<model.numverts ; i++)
	{
		...
		{
			logerror ("Vertex with no non-degenerate triangles attached"); // jkrige - was Error()
		}
	...
	if (framecount >= MAXFRAMES)
		logerror ("Too many frames; increase MAXFRAMES"); // jkrige - was Error()
...
void Cmd_SkinGroupStart (void)
{
	...
	while (1)
	{
		GetToken (true);
		if (endofscript)
			logerror ("End of file during group"); // jkrige - was Error()
		...
		{
			logerror ("$skin or $skingroupend expected\n"); // jkrige - was Error()
		}

	}

	if (skins[groupskin].numgroupskins == 0)
		logerror ("Empty group\n"); // jkrige - was Error()
}
...
void Cmd_FrameGroupStart (void)
{
	...
	while (1)
	{
		...
		{
			logerror ("$frame or $framegroupend expected\n"); // jkrige - was Error()
		}
...
static void ParseScript (void)
{
	while (1)
	{
		...
		{
			if (cdset)
				logerror ("Two $cd in one model"); // jkrige - was Error()
			cdset = true;
			GetToken (false);
...
static void LoadPCXSkin(char *filename, byte **buffer)
{
	...
	{
		logprint("->tag = %d [10]\n", pcx->tag);
		logprint("->version = %d [5]\n", pcx->version);
		logprint("->encoding = %d [1]\n", pcx->encoding);
		logprint("->pixelBits = %d [8]\n", pcx->pixelBits);
		logprint("->planes = %d [1]\n", pcx->planes);
		logprint("->lineBytes = %d\n", pcx->lineBytes);
		logprint("width = %d [640]\n", w);
		logprint("height = %d [480]\n", h);
		logerror ("Bad PCX skin file.\n"); // jkrige - was Error()
	}

	// Allocate page
	if((dst = malloc(SKINPAGE_SIZE)) == NULL)
	{
		logerror ("Failed to allocate memory for skin page.\n"); // jkrige - was Error()
	}
	*buffer = dst;
		...
		i++)
	{
		...
		{
			logerror ("PCX decompression overflow.\n"); // jkrige - was Error()
		}
	}
	free(pcx);
}
