	...
	int *triangleCount)
{
	...
	for(i = 0; i < vertexCount; i++)
	{
		...
		if(tk_IntNumber != i)
		{
			logerror ("File '%s', line %d:\nVertex index mismatch.\n",
				tk_SourceName, tk_Line); // jkrige - was Error()
		}
	...
	{
		logerror ("Too many triangles in file %s\n", InputFileName); // jkrige - was Error()
	}
	...
	for(i = 0; i < triCount; i++)
	{
		...
		if(tk_IntNumber != 3)
		{
			logerror ("File '%s', line %d:\nBad polygon vertex count: %d.",
				tk_SourceName, tk_Line, tk_IntNumber); // jkrige - was Error()
		}
		for(j = 0; j < 3; j++)
		{
			TK_BeyondRequire(TK_LBRACKET, TK_INTNUMBER);
			if(tk_IntNumber != j)
			{
				logerror ("File '%s', line %d:\nTriangle vertex index"
					...
					tk_IntNumber, j); // jkrige - was Error()
			}
	...
	int *triangleCount)
{
	...
	if(tk_IntNumber != 1)
	{
		logerror ("Unsupported version (%d) in file %s\n", tk_IntNumber,
			InputFileName); // jkrige - was Error()
	}
...
static void _3DSError(char *message)
{
	logerror (message); // jkrige - was Error()
}
...
*/
			...
			{
				logerror ("Error: too many triangles; increase MAXTRIANGLES\n"); // jkrige - was Error()
			}
		}
	}
