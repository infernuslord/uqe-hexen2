...
qboolean CanJoinFaces (face_t *f, face_t *f2)
{
	...
	{	// does this ever happen? theyy shouldn't share.
		logprint ("CanJoinFaces: edge with different contents");
		return false;
	}
...
void RecursiveGrowRegion (dface_t *r, face_t *f)
{
	...
	if (f->outputnumber != -1)
		logerror ("RecursiveGrowRegion: region collision"); // jkrige - was Error()
	...
	for (i=0 ; i<f->numpoints ; i++)
	{
		...
		// emit a surfedge
			if (numsurfedges == MAX_MAP_SURFEDGES)
				logerror ("numsurfedges == MAX_MAP_SURFEDGES"); // jkrige - was Error()
...
// extend e1 to e2
	...
	if (ed->v[0] == ed2->v[0])
		...
		logerror ("HealEdges: edges don't meet"); // jkrige - was Error()

	VectorSubtract (dvertexes[ed->v[1]].point, dvertexes[ed->v[0]].point, v2);
	VectorNormalize (v2);
	
	if (!VectorCompare (v1, v2))
		logerror ("HealEdges: edges not colinear"); // jkrige - was Error()
...
// remove all uses of e2
	...
	if (saved != 2)
		logprint ("WARNING: didn't find both faces for a saved edge\n");
	else
	{
		for (i=0 ; i<2 ; i++)
		{	// remove this edge
...
//	qprintf ("%5i c2\n", c2);
//	qprintf ("%5i c3+\n", c3);
	logprint ("%5i deges removed by tjunction healing\n", c2);
}
...
//			continue;	// allready grown into an earlier region
	...
	// emit a region
		...
		if (numfaces == MAX_MAP_FACES)
			logerror ("MAX_MAP_FACES"); // jkrige - was Error()
		f->outputnumber = numfaces;
		r = &dfaces[numfaces];
