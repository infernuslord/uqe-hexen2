...
void CanonicalVector (vec3_t vec)
{
	...
	else if (vec[2] < -EQUAL_EPSILON)
	{
		...
		vec[2] = 0;
	logerror ("CanonicalVector: degenerate"); // jkrige - was Error()
}
...
wedge_t	*FindEdge (vec3_t p1, vec3_t p2, double *t1, double *t2)
{
	...
	*t2 = DotProduct (p2, dir);
	...
	if (numwedges == MAXWEDGES)
		logerror ("FindEdge: numwedges == MAXWEDGES"); // jkrige - was Error()
	w = &wedges[numwedges];
	numwedges++;
...
// insert a new wvert before v
	if (numwverts == MAXWVERTS)
		logerror ("AddVert: numwverts == MAXWVERTS"); // jkrige - was Error()
...
restart:	
	...
	// past lastcorner
		...
		if (f->original)
			logerror ("SplitFaceForTjunc: f->original"); // jkrige - was Error()
...
// origin points won't allways be inside the map, so extend the hash area 
	...
	logprint ("%i world edges  %i edge points\n", numwedges, numwverts);
...
//
	...
	logprint ("%i edges added by tjunctions\n", tjuncs);
	logprint ("%i faces added by tjunctions\n", tjuncfaces);
}
