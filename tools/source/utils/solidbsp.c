...
//
	...
	if (!bestsurface)
	{
		...
		logerror ("ChooseMidPlaneFromList: no valid planes"); // jkrige - was Error()
	}
...
void CalcSurfaceInfo (surface_t *surf)
{
	...
	if (!surf->faces)
		logerror ("CalcSurfaceInfo: surface without a face"); // jkrige - was Error()
...
logerror ("Bad contents"); // jkrige - was Error()
...
void LinkConvexFaces (surface_t *planelist, node_t *leafnode)
{
	...
	for ( surf = planelist ; surf ; surf = surf->next)
	{
		for (f = surf->faces ; f ; f=f->next)
		{
			...
			if (!leafnode->contents)
				...
				logerror ("Mixed face contents in leafnode"); // jkrige - was Error()
		}
	}
	...
	default:
		logerror ("LinkConvexFaces: bad contents number"); // jkrige - was Error()
	}
...
//			MergePlaneFaces (backfrag);
		...
		{
			if (!frontfrag->faces)
				logerror ("surface with no faces"); // jkrige - was Error()
			frontfrag->next = frontlist;
			frontlist = frontfrag;
		}
		if (backfrag)
...
//
	...
	logprint ("%5i split nodes\n", splitnodes);
	logprint ("%5i solid leafs\n", c_solid);
	logprint ("%5i empty leafs\n", c_empty);
	logprint ("%5i water leafs\n", c_water);	
	logprint ("%5i leaffaces\n",leaffaces);
	logprint ("%5i nodefaces\n", nodefaces);
