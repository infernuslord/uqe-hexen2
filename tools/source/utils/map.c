...

{
	...
	if (nummiptex == MAX_MAP_TEXINFO)
		...
		logerror ("nummiptex == MAX_MAP_TEXINFO"); // jkrige - was Error()
...
// allocate a new texture
	...
	if (numtexinfo == MAX_MAP_TEXINFO)
		...
		logerror ("numtexinfo == MAX_MAP_TEXINFO"); // jkrige - was Error()
...
skipspace:
...

	{
...

		{
			...
			if (!crossline)
				...
				logerror ("Line %i is incomplete",scriptline); // jkrige - was Error()
...

	{
		...
		if (!crossline)
			...
			logerror ("Line %i is incomplete\n",scriptline); // jkrige - was Error()
...

	{
...

		{
			...
			if (!*script_p)
				...
				logerror ("EOF inside quoted token"); // jkrige - was Error()

			*token_p++ = *script_p++;

			if (token_p > &token[MAXTOKEN-1])

				logerror ("Token too large on line %i",scriptline); // jkrige - was Error()
...

{
	...
	if (strlen(token) >= MAX_KEY-1)
		...
		logerror ("ParseEpar: token too long"); // jkrige - was Error()
...

{
...

	{
...

		{
			...
			logprint ("WARNING: entity %i, brush %i: origin brush isn't axial\n"
...

{
	...
	if (strcmp (token, "{") )
		...
		logerror ("ParseEntity: { not found"); // jkrige - was Error()

	

	if (num_entities == MAX_MAP_ENTITIES)

		logerror ("num_entities == MAX_MAP_ENTITIES"); // jkrige - was Error()
...

	{
		...
		if (!GetToken (true))
			...
			logerror ("ParseEntity: EOF without closing brace"); // jkrige - was Error()
