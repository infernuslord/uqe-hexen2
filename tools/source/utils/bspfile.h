...
#define	MAX_MAP_PLANES		65536 // jkrige - design bounds : 16384
...
#define IDWALHEADER	(('L'<<24)+('A'<<16)+('W'<<8)+'H') // jkrige - external .wal texture loading
		// little-endian "HWAL"

#define WALVERSION	1 // jkrige - external .wal texture loading

typedef struct miptex_wal_s
{
	int			ident; // jkrige - external .wal texture loading
	int			version; // jkrige - external .wal texture loading
	char		name[32];
	unsigned	width, height;
	unsigned	offsets[MIPLEVELS];		// four mip maps stored
	//char		animname[32];			// next frame in animation chain // jkrige - external .wal texture loading
	//int			flags; // jkrige - external .wal texture loading
	//int			contents; // jkrige - external .wal texture loading
	//int			value; // jkrige - external .wal texture loading
} miptex_wal_t;
// jkrige - .wal texture support
