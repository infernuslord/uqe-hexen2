...
winding_t *OldNewWinding (int points)
{
	...
	if (points > MAX_POINTS_ON_WINDING)
		logerror ("NewWinding: %i points", points); // jkrige - was Error()
...
// determine sides for each point
	...
	if (neww->numpoints > maxpts)
		logerror ("ClipWinding: points exceeded estimate"); // jkrige - was Error()
...
#ifdef __alpha
#ifdef _WIN32
LPVOID LeafThread (LPVOID thread, int thrnum)
#else
pthread_addr_t LeafThread (pthread_addr_t thread)
#endif
#else
void *LeafThread (int thread, int thrnum)
#endif
// jkrige - threads
{
	portal_t        *p;
	...
	logprint ("Begining LeafThread: %i\n",(int)thread);
	do
	{
		p = GetNextPortal ();
		if (!p)
		...
		//jkrige - progress
		//if(thrnum == 0)
		//	printf("%i of %i: %i%%\r", cnt, numportals*2, 50*cnt/numportals);
		//if(thrnum == 1)
		//	printf("\n%i of %i: %i%%\r", cnt, numportals*2, 50*cnt/numportals);
		//if(thrnum == 2)
		//	printf("\n\n%i of %i: %i%%\r", cnt, numportals*2, 50*cnt/numportals);
		//if(thrnum == 3)
		//	printf("\n\n\n%i of %i: %i%%\r", cnt, numportals*2, 50*cnt/numportals);
	...
	logprint ("Completed LeafThread: %i\n",(int)thread);

	cnt--;
	logprint ("%i of %i: %i%%\n", cnt, numportals*2, 50*cnt/numportals);
...
//
	...
	for (i=0 ; i<leaf->numportals ; i++)
	{
		...
		if (p->status != stat_done)
			logerror ("portal not done"); // jkrige - was Error()
		for (j=0 ; j<bitbytes ; j++)
			outbuffer[j] |= p->visbits[j];
	}

	if (outbuffer[leafnum>>3] & (1<<(leafnum&7)))
		logerror ("Leaf portals saw into leaf"); // jkrige - was Error()
...
//
	...
	if (vismap_p > vismap_end)
		logerror ("Vismap expansion overflow"); // jkrige - was Error()
...
#ifdef _WIN32
{
	...
	for (i=0 ; i<numthreads-1 ; i++)
	{
		...
		if (work_threads[i] == NULL)
			logerror ("pthread_create failed"); // jkrige - was Error()
	}

	// jkrige - threads
	//LeafThread((LPVOID)(numthreads-1));
	LeafThread((LPVOID)(numthreads-1),(numthreads-1));
	// jkrige - threads
	for (i=0 ; i<numthreads-1 ; i++)
	{
...
#else
{
	...
	if (pthread_mutexattr_create (&mattrib) == -1)
		logerror ("pthread_mutex_attr_create failed"); // jkrige - was Error()
	if (pthread_mutexattr_setkind_np (&mattrib, MUTEX_FAST_NP) == -1)
		logerror ("pthread_mutexattr_setkind_np failed"); // jkrige - was Error()
	if (pthread_mutex_init (my_mutex, mattrib) == -1)
		logerror ("pthread_mutex_init failed"); // jkrige - was Error()

	if (pthread_attr_create (&attrib) == -1)
		logerror ("pthread_attr_create failed"); // jkrige - was Error()
	if (pthread_attr_setstacksize (&attrib, 0x100000) == -1)
		logerror ("pthread_attr_setstacksize failed"); // jkrige - was Error()
	...
	for (i=0 ; i<numthreads ; i++)
	{
		if (pthread_join (work_threads[i], &status) == -1)
			logerror ("pthread_join failed"); // jkrige - was Error()
	}

	if (pthread_mutex_destroy (my_mutex) == -1)
			logerror ("pthread_mutex_destroy failed"); // jkrige - was Error()
}
#endif  // _win32
#else
	// jkrige - threads
...
void LoadPortals (char *name)
{
	...
	else
	{
		...
		if (!f)
		{
			logprint ("LoadPortals: couldn't read %s\n",name);
	...
	if (fscanf (f,"%79s\n%i\n%i\n",magic, &portalleafs, &numportals) != 3)
		logerror ("LoadPortals: failed to read header"); // jkrige - was Error()
	if (strcmp(magic,PORTALFILE))
		logerror ("LoadPortals: not a portal file"); // jkrige - was Error()
...
// each file portal is split into two memory portals
	...
	for (i=0, p=portals ; i<numportals ; i++)
	{
		if (fscanf (f, "%i %i %i ", &numpoints, &leafnums[0], &leafnums[1])
			!= 3)
			logerror ("LoadPortals: Error reading portal %i", i); // jkrige - was Error()
		if (numpoints > MAX_POINTS_ON_WINDING)
			logerror ("LoadPortals: portal %i has too many points", i); // jkrige - was Error()
		if ( leafnums[0] > portalleafs
		|| leafnums[1] > portalleafs)
			logerror ("LoadPortals: portal %i, leafnums > portalleafs", i); // jkrige - was Error()
		...
		for (j=0 ; j<numpoints ; j++)
		{
			...
			, &w->points[j][0], &w->points[j][1], &w->points[j][2]) != 3)
				logerror ("LoadPortals: reading portal %i", i); // jkrige - was Error()
		}
		fscanf (f, "\n");
	...
	// create forward portal
		...
		if (l->numportals == MAX_PORTALS_ON_LEAF)
			logerror ("Leaf with too many portals"); // jkrige - was Error()
...
int main (int argc, char **argv)
{
	...
	// jkrige
	SetConsoleTitle("UQE Hexen II VIS");

	init_log("uqehx2vis.log");
	logprint("UQEHX2VIS 1.16 -- based on VIS by John Carmack\n");
    logprint("===============================================\n");
	...
	for (i=1 ; i<argc ; i++)
	{
		...
		{
			logprint ("Gil's optimizations disabled\n");
			GilMode=0;
		}
		...
		else if (!strcmp(argv[i], "-v"))
		{
			...
			logerror ("uqehx2vis: Unknown option \"%s\"", argv[i]); // jkrige - was Error()
		else
			break;
	}
	...
	if (i != argc - 1)
		logerror ("usage: uqehx2vis [-nogil] [-threads #] [-level 0-4] [-fast] [-v] bspfile"); // jkrige - was Error()
	// jkrige
