...
void CheckStack (leaf_t *leaf, threaddata_t *thread)
{
	...
	for (p=thread->pstack_head.next ; p ; p=p->next)
		if (p->leaf == leaf)
			logerror ("CheckStack: leaf recursion"); // jkrige - was Error()
}
...
{
	if (Stats[0] ==0)
		logprint("printstats:  no stats\n");
	else
		logprint("%f %% rejected by gilmode\n",100.0*((float)Stats[1])/(float)(Stats[0]+Stats[1]));
}

double ComputeBoundingSphere(winding_t *source,vec3_t c)
{
	int i;
	double r,tr;
	vec3_t dif;
...
void PortalFlow (portal_t *p)
{
	...
	if (p->status != stat_working)
		logerror ("PortalFlow: reflowed"); // jkrige - was Error()
...
void BasePortalVis (void)
{
	...
	{
		//jkrige - progress
		printf("%i of %i: %i%%\r", i, numportals*2, 50*i/numportals);
		fflush(stdout);
		//jkrige - progress
		...
		for (j=0, tp = portals ; j<num2 ; j++, tp++)
		{
			...
			{
				d = (float)DotProduct (w->points[k], p->plane.normal)
					- p->plane.dist;
				if (d > ON_EPSILON)
					break;
			}
			...
			for (k=0 ; k<w->numpoints ; k++)
			{
				d = (float)DotProduct (w->points[k], tp->plane.normal)
					- tp->plane.dist;
				if (d < -ON_EPSILON)
					break;
			}
...
//		logprint ("portal:%4i  c_leafsee:%4i \n", i, c_leafsee);
	
	}

	logprint ("%i of %i: %i%%\n", i, numportals*2, 50*i/numportals);
