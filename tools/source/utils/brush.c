...
void CheckFace (face_t *f)
{
	...
	if (f->numpoints < 3)
		logerror ("CheckFace: %i points",f->numpoints); // jkrige - was Error()
	...
	for (i=0 ; i<f->numpoints ; i++)
	{
		...
		for (j=0 ; j<3 ; j++)
			if (p1[j] > BOGUS_RANGE || p1[j] < -BOGUS_RANGE)
				logerror ("CheckFace: BUGUS_RANGE: %f",p1[j]); // jkrige - was Error()
...
//			logprint ("CheckFace: point #%i off plane\n",i);
...
*/
	...
	// check the edge isn't degenerate
		...
		if (VectorLength (dir) < ON_EPSILON)
			logerror ("CheckFace: degenerate edge"); // jkrige - was Error()
	...
	// all other points must be on front side
		for (j=0 ; j<f->numpoints ; j++)
		{
			...
			if (d > edgedist)
				logerror ("CheckFace: non-convex"); // jkrige - was Error()
		}
	}
}
...
// NOTE: should these have an epsilon around 1.0?		
	...
	if (normal[0] == -1.0 ||
		normal[1] == -1.0 ||
		normal[2] == -1.0)
		logerror ("PlaneTypeForNormal: not a canonical vector"); // jkrige - was Error()
...
int	FindPlane (plane_t *dplane, int *side)
{
	...
	if (dot < 1.0 - ANGLEEPSILON || dot > 1.0 + ANGLEEPSILON)
		logerror ("FindPlane: normalization error"); // jkrige - was Error()
	...
	if (numbrushplanes == MAX_MAP_PLANES)
		logerror ("numbrushplanes == MAX_MAP_PLANES"); // jkrige - was Error()
...
void CreateBrushFaces (void)
{
	...
	// this face is a keeper
		...
		if (f->numpoints > MAXEDGES)
			logerror ("f->numpoints > MAXEDGES"); // jkrige - was Error()
...
{ {-24,-24,-20}, {24,24,20} },
{ {-16,-16,-16}, {16,16,12} },
{ {-8,-8,-8}, {8,8,8} },	//{ {-40,-40,-42}, {40,40,42} },	//called hyrda, but not really
{ {-28,-28,-40}, {28,28,40} }	//{ {-48,-48,-50}, {48,48,50} }
};
...
void AddBrushPlane (plane_t *plane)
{
	...
	if (numbrushfaces == MAX_FACES)
		logprint ("AddBrushPlane: numbrushfaces == MAX_FACES\n");
//		Error ("AddBrushPlane: numbrushfaces == MAX_FACES");
	l = VectorLength (plane->normal);
	if (l < 0.999 || l > 1.001)
		logerror ("AddBrushPlane: bad normal"); // jkrige - was Error()
...
int AddHullPoint (vec3_t p, int hullnum)
{
	...
	if (num_hull_points == MAX_HULL_POINTS)
		logerror ("MAX_HULL_POINTS"); // jkrige - was Error()
...
void AddHullEdge (vec3_t p1, vec3_t p2, int hullnum)
{
	...
	if (num_hull_edges == MAX_HULL_EDGES)
		logerror ("MAX_HULL_EDGES"); // jkrige - was Error()
