...
//
	if (numplanes == MAX_MAP_PLANES)
		logerror ("numplanes == MAX_MAP_PLANES"); // jkrige - was Error()
...
//
	...
	// emit a marksurface
		if (nummarksurfaces == MAX_MAP_MARKSURFACES)
			logerror ("nummarksurfaces == MAX_MAP_MARKSURFACES"); // jkrige - was Error()
		f = *fp;
		do
		{
...
// emit a node	
	if (numnodes == MAX_MAP_NODES)
		logerror ("numnodes == MAX_MAP_NODES"); // jkrige - was Error()
	n = &dnodes[numnodes];
	numnodes++;
...
void WriteDrawNodes (node_t *headnode)
{
	...
	if (headnode->contents < 0)
		logerror ("FinishBSPModel: empty model"); // jkrige - was Error()
#endif

// emit a model
	if (nummodels == MAX_MAP_MODELS)
		logerror ("nummodels == MAX_MAP_MODELS"); // jkrige - was Error()
...
void	TEX_InitFromWad (char *path)
{
	...
	if (strncmp (wadinfo.identification, "WAD2", 4))
		logerror ("TEX_InitFromWad: %s isn't a wadfile",path); // jkrige - was Error()
...
int LoadLump (char *name, byte *dest)
{
	...
	for (i=0 ; i<wadinfo.numlumps ; i++)
	{
		...
		if (!strcmp(strlwr(cname), strlwr(lumpinfo[i].name))) // jkrige - case sensitive!!
		{
	...
	logprint ("WARNING: texture %s not found\n", name);
	return 0;
}
...
void AddAnimatingTextures (void)
{
	...
	for (i=0 ; i<base ; i++)
	{
		...
		// see if this name exists in the wadfile
			for (k=0 ; k<wadinfo.numlumps ; k++)
			{
				...
				if (!strcmp(strlwr(name), strlwr(lumpinfo[k].name))) // jkrige - case sensitive!!
				{
	...
	logprint ("added %i texture frames\n", nummiptex - base);
}

// jkrige - .wal texture support
void WriteWALFile(char *bspfilename)
{
	int i;
	dmiptexlump_t	*l;
	char texname[32];
	...
	shiftoffset = sizeof(qtex->ident) + sizeof(qtex->version) + (sizeof(qtex->name) - sizeof(mt->name));

	if (strlen(projectpath) != 0)
		strcpy(projpath, projectpath);
	else
		strcpy(projpath, "./");

	logprint ("Extracting HWAL textures...\n");

	sprintf(fullpath, "%stextures", projpath);
	Q_mkdir (fullpath);
	...
	for (i=0 ; i<m->nummiptex ; i++)
	{
		...
		qtex = malloc (sizeof(miptex_wal_t) + pixels);
		memset (qtex, 0, sizeof(miptex_wal_t));

		qtex->ident = LittleLong (IDWALHEADER);
		qtex->version = WALVERSION;
		qtex->width = mt->width;
		qtex->height = mt->height;

		strcpy(qtex->name, mt->name);

		qtex->offsets[0] = LittleLong (mt->offsets[0] + shiftoffset);
		qtex->offsets[1] = LittleLong (mt->offsets[1] + shiftoffset);
		qtex->offsets[2] = LittleLong (mt->offsets[2] + shiftoffset);
		qtex->offsets[3] = LittleLong (mt->offsets[3] + shiftoffset);

		memcpy (qtex+1,  (byte *)m + m->dataofs[i] + sizeof(miptex_t), pixels);

		// save file
		if(mt->name[0] == '*')
		{
			sprintf (fullpath, "%stextures/%s.wal", projpath, strnset(mt->name, '-', 1));
			logprint ("writing %s.wal\n", strnset(mt->name, '-', 1));
		}
		else
		{
			sprintf (fullpath, "%stextures/%s.wal", projpath, mt->name);
			logprint ("writing %s.wal\n", mt->name);
		}

		SaveFile (fullpath, (byte *)qtex, sizeof(miptex_wal_t) + pixels);
...
void WriteMiptex (void)
{
	...
	if (!path || !path[0])
	{
		logprint ("WARNING: no wadfile specified\n");
		texdatasize = 0;
		return;
	}*/
	...
	for (i=0 ; i<nummiptex ; i++)
	{
		...
		if (data + len - dtexdata >= MAX_MAP_MIPTEX)
			logerror ("Textures exceeded MAX_MAP_MIPTEX"); // jkrige - was Error()
		if (!len)
...
{
	logprint ("--- FinishBSPFile ---\n");
	logprint ("WriteBSPFile: %s\n", bspfilename);
