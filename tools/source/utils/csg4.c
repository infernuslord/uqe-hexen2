...
void SplitFace (face_t *in, plane_t *split, face_t **front, face_t **back)
{
	...
	if (in->numpoints < 0)
		logerror ("SplitFace: freed face"); // jkrige - was Error()
...
// distribute the points and generate splits
	...
	for (i=0 ; i<in->numpoints ; i++)
	{
		if (newf->numpoints > MAXEDGES || new2->numpoints > MAXEDGES)
			logerror ("SplitFace: numpoints > MAXEDGES"); // jkrige - was Error()
...
#if 0
{
	...
	for (i=0 ; i<f->numpoints ; i++)
		logprint ("(%f,%f,%f) ",f->pts[i][0], f->pts[i][1], f->pts[i][2]);
	logprint ("\n");
}
#endif
		newf = AllocFace ();
...
#if 0
	if (!csgfaces)
		logerror ("No faces"); // jkrige - was Error()
#endif

	surfhead = BuildSurfaces ();
	
	logprint ("%5i brushfaces\n", brushfaces);
	logprint ("%5i csgfaces\n", csgfaces);
	logprint ("%5i mergedfaces\n", csgmergefaces);
