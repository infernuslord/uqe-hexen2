...
qboolean FillOutside (node_t *node)
{
	...
	if (!inside)
	{
		logprint ("Hullnum %i: No entities in empty space -- no filling performed\n", hullnum);
		return false;
	}
...
// first check to see if an occupied leaf is hit
	...
	if (!hullnum)
	{
		...
		if (!leakfile)
			logerror ("Couldn't open %s\n", pointfilename); // jkrige - was Error()
	}
	...
	if (RecursiveFillOutside (outside_node.portals->nodes[s], false))
	{
		...
		logprint ("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		logprint ("reached occupant at: (%4.0f,%4.0f,%4.0f)\n", v[0], v[1], v[2]);
		logprint ("no filling performed\n");
		if (!hullnum)
			fclose (leakfile);
		logprint ("leak file written to %s\n", pointfilename);			
		logprint ("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		return false;
	}
	if (!hullnum)
