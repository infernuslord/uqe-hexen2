...
#include "bsp5.h"
#include "cmdlib.h" // jkrige - .wal texture support
...
qboolean noclip;
qboolean onlyents;
qboolean onlytextures; // jkrige - .wal texture support
qboolean textures; // jkrige - .wal texture support
...
// find the major axis
	...
	if (x==-1)
		logerror ("BaseWindingForPlane: no axis found"); // jkrige - was Error()
...
//
	...
	{
		PrintEntity (ent);
		logerror ("Entity with no valid brushes"); // jkrige - was Error()
	}
...
void UpdateEntLump (void)
{
	...
	logprint ("Updating entities lump...\n");
...
void WriteClipHull (void)
{
	...
	logprint ("---- WriteClipHull ----\n");
	logprint ("Writing %s\n", hullfilename);

	f = fopen (hullfilename, "w");
	if (!f)
		logerror ("Couldn't open %s", hullfilename); // jkrige - was Error()
...
void ReadClipHull (int hullnum)
{
	...
	if (fscanf (f,"%i\n", &n) != 1)
		logerror ("Error parsing %s", hullfilename); // jkrige - was Error()

	if (n != nummodels)
		logerror ("ReadClipHull: hull had %i models, base had %i", n, nummodels); // jkrige - was Error()
	...
	for (i=0 ; i<n ; i++)
	{
		if (numclipnodes == MAX_MAP_CLIPNODES)
			logerror ("ReadClipHull: MAX_MAP_CLIPNODES"); // jkrige - was Error()
...
#else	//alpha
	logprint ("forking hull processes...\n");
// fork a process for each clipping hull
	fflush (stdout);
	if (!fork ())
	{
...
// create the hulls sequentially
	logprint ("building hulls sequentially...\n");
...
/*		if (sourcebase[0] == '/')
	...
	// scan for the maps directory
		...
		if (!scan)
			logerror ("Couldn't identify project directory from pathname.  Use -proj <path>"); // jkrige - was Error()
	}

	logprint ("Project directory: %s\n", projectpath);
}
...
int main (int argc, char **argv)
{
	...
	// jkrige
	SetConsoleTitle("UQE Hexen II BSP");

	init_log("uqehx2bsp.log");
	logprint("UQEHX2BSP 1.16 -- based on QBSP by John Carmack\n");
    logprint("===============================================\n");
...
//
	for (i=1 ; i<argc ; i++)
	{
		if (argv[i][0] != '-')
			...
			onlyents = true;
		else if (!strcmp (argv[i],"-onlytextures")) // jkrige - .wal texture support
			onlytextures = true;
		else if (!strcmp (argv[i],"-textures")) // jkrige - .wal texture support
			textures = true;
		...
		else if (!strcmp (argv[i],"-proj"))
		{
			...
			logerror ("uqehx2bsp: Unknown option '%s'", argv[i]); // jkrige - was Error()
	}
	...
	if (i != argc - 2 && i != argc - 1)
		logerror ("usage: uqehx2bsp [options] sourcefile [destfile]\noptions: -notjunc -nofill -draw -onlyents -verbose -proj <projectpath>"); // jkrige - was Error()
	// jkrige
