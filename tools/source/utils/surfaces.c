...
if ( !size[2] && size[0] <= 496 && size[1] <= 496 && size[0]*size[1]<0x10000)
	return;
		...
		// split it
			...
			if (!front || !back)
				logerror ("SubdivideFace: didn't split the polygon"); // jkrige - was Error()
			*prevptr = front;
			front->next = back;
			back->next = next;
			f = front;
...
void SubdivideFaces (surface_t *surfhead)
{
	...
	logprint ("%i faces added by subdivision\n", subdivides);
...
int	GetVertex (vec3_t in, int planenum)
{
	...
	if (hv->num==MAX_MAP_VERTS)
		logerror ("GetVertex: MAX_MAP_VERTS"); // jkrige - was Error()
	hvert_p++;
		
// emit a vertex
	if (numvertexes == MAX_MAP_VERTS)
		logerror ("numvertexes == MAX_MAP_VERTS"); // jkrige - was Error()
...
int GetEdge (vec3_t p1, vec3_t p2, face_t *f)
{
	...
	if (!f->contents[0])
		logerror ("GetEdge: 0 contents"); // jkrige - was Error()
...
// emit an edge
	if (numedges == MAX_MAP_EDGES)
		logerror ("numedges == MAX_MAP_EDGES"); // jkrige - was Error()
...
void FindFaceEdges (face_t *face)
{
	...
	if (face->numpoints > MAXEDGES)
		logerror ("WriteFace: %i points", face->numpoints); // jkrige - was Error()
...
void CheckVertexes (void)
{
	...
	logprint ("%5i bad edge points\n", cb);
	logprint ("%5i 0 edge points\n", c0);
	logprint ("%5i 2 edge points\n", c1);
	logprint ("%5i 4 edge points\n", c2);
	logprint ("%5i 6+ edge points\n", c3);
}
...
//	logprint ("%5i edges\n", i);
//	logprint ("%5i c_nonconvex\n", c_nonconvex);
//	logprint ("%5i c_multitexture\n", c_multitexture);
