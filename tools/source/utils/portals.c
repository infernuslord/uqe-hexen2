...
void AddPortalToNodes (portal_t *p, node_t *front, node_t *back)
{
	if (p->nodes[0] || p->nodes[1])
		logerror ("AddPortalToNode: allready included"); // jkrige - was Error()
...
// remove reference to the current portal
	...
	while (1)
	{
		t = *pp;
		if (!t)
			logerror ("RemovePortalFromNode: portal not in leaf"); // jkrige - was Error()
		...
		if (t->nodes[0] == l)
			...
			logerror ("RemovePortalFromNode: portal not bounding leaf"); // jkrige - was Error()
	}
...
void CheckWindingInNode (winding_t *w, node_t *node)
{
	...
	for (i=0 ; i<w->numpoints ; i++)
	{
		for (j=0 ; j<3 ; j++)
			...
			{
				logprint ("WARNING: CheckWindingInNode: outside\n");
				return;
			}
	}
}
...
void CheckWindingArea (winding_t *w)
{
	...
	if (total < 16)
		logprint ("WARNING: winding area %f\n", total);
}
...
void CheckLeafPortalConsistancy (node_t *node)
{
	...
	for (p = node->portals ; p ; p = p->next[side])	
	{
		if (p->nodes[0] == node)
			...
			logerror ("CutNodePortals_r: mislinked portal"); // jkrige - was Error()
...
//
	...
	for (p = node->portals ; p ; p = p->next[side])	
	{
		...
		if (!w)
		{
			logprint ("WARNING: CutNodePortals_r:new portal was clipped away\n");
			break;
		}
	}
...
// write the file
	...
	if (!pf)
		logerror ("Error opening %s", portfilename); // jkrige - was Error()
