...
cvar_t		scr_conspeed = {"scr_conspeed","600"}; // jkrige - was 300
...
void SCR_TileClear (void)
{
	...
	if(cl.intermission)
	{
		// jkrige - scale2d
		if(vid_scale2d.value)
			Draw_TileClear (0, 0, modelist[(int)vid_mode.value].width, modelist[(int)vid_mode.value].height);
		else
...
void SB_IntermissionOverlay(void)
{
	...
	//Draw_TileClear(0, 0, vid.width, vid.height); // jkrige - intermission tileclear
	// jkrige - intermission tileclear
	SCR_TileClear();
	// jkrige - intermission tileclear
	...
	else if (cl.intermission == 12)
	{
		...
		//		this is here to prevent the PoP intro from redrawing
		//		itself too fast with the new customizable FPS cap
		elapsed = (Sys_FloatTime() - introTime) * 20;
		//elapsed = (introTime);
		//if (introTime < 500)
		//	introTime+=0.25;
		// jkrige - configurable fps caps
...
void SCR_UpdateScreen (void)
{
	...
	// jkrige - field of view (fov) fix
	//if (oldfov != scr_fov.value || oldscreensize != scr_viewsize.value)
	//	vid.recalc_refdef = true;
	if (oldfov != fov_width || oldscreensize != scr_viewsize.value)
		vid.recalc_refdef = true;
	// jkrige - field of view (fov) fix


	//if (oldscreensize != scr_viewsize.value) // jkrige?
	//	vid.recalc_refdef = true;
	// jkrige - sbar height
...
/*	else if (cl.intermission == 2 && key_dest == key_game)
	...
	else
	{
		if (crosshair.value)
		{
			...
			Draw_Character (vid.width / 2, vid.height / 2, '+');
			// jkrige - scale2d
		}
		...
		if (info_up)
		{
			// jkrige - objectives only shows in Mission Pack
