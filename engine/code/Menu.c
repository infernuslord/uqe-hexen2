...
// jkrige - scale2d
#if GLQUAKE
qboolean param_scale2d_override = false;
#endif
// jkrige - scale2d
...
void M_Difficulty_Draw (void)
{
	...
	{
		if (setup_class < 1 || setup_class > NUM_CLASSES-1)
...
void M_Class_Draw (void)
{
	...
	{
		if (m_class_cursor == NUM_CLASSES-1)
			m_class_cursor = 0;
...
void M_SinglePlayer_Draw (void)
{
	...
	//if(m_oldmission.value && m_singleplayer_cursor > 2)
	if(!hexen2_portals && m_singleplayer_cursor > 2) // jkrige - portals game
		m_singleplayer_cursor = 0;

	ScrollTitle("gfx/menu/title1.lmp");
	
	// jkrige - mission select
	//M_DrawBigString (72,60+(0*20),"NEW MISSION");
	//if (m_oldmission.value)
	if(hexen2_portals) // jkrige - portals game
		M_DrawBigString (72,60+(0*20),"NEW MISSION");
	else
		M_DrawBigString (72,60+(0*20),"MISSION");
	// jkrige - mission select
...
void M_SinglePlayer_Key (int key)
{
	...
	case K_UPARROW:
		...
		{
			if (--m_singleplayer_cursor < 0)
				m_singleplayer_cursor = SINGLEPLAYER_ITEMS - 3;
...
void M_Load_Draw (void)
{
	...
	for (i=0 ; i< MAX_SAVEGAMES; i++)
		M_Print (64 /*16*/, 60 + 8*i, m_filenames[i]); // jkrige - load/save text alignment

// line cursor
	M_DrawCharacter (56 /*8*/, 60 + load_cursor*8, 12+((int)(realtime*4)&1));
}
...
int		setup_cursor = 5;
// jkrige - multiplayer setup menu alignment
//int		setup_cursor_table[] = {40, 56, 80, 104, 128, 156};
int		setup_cursor_table[] = {68, 84, 108, 132, 156, 184};
// jkrige - multiplayer setup menu alignment
...
void M_Setup_Draw (void)
{
	...
	M_Print (64, 68-VertAdjust, "Hostname");
	M_DrawTextBox (160, 60-VertAdjust, 16, 1);
	M_Print (168, 68-VertAdjust, setup_hostname);

	M_Print (64, 84-VertAdjust, "Your name");
	M_DrawTextBox (160, 76-VertAdjust, 16, 1);
	M_Print (168, 84-VertAdjust, setup_myname);

	M_Print (64, 108-VertAdjust, "Current Class: ");
	M_Print (88, 116-VertAdjust, ClassNames[setup_class-1]);

	M_Print (64, 132-VertAdjust, "First color patch");
	M_Print (64, 156-VertAdjust, "Second color patch");
	
	M_DrawTextBox (64, 176-VertAdjust, 14, 1);
	M_Print (72, 184-VertAdjust, "Accept Changes");

	p = Draw_CachePic (va("gfx/menu/netp%i.lmp",setup_class));
	M_BuildTranslationTable(setup_top, setup_bottom);

	/* garymct */
	M_DrawTransPicTranslate (220, 100-VertAdjust, p);

	M_DrawCharacter (56, setup_cursor_table [setup_cursor]-VertAdjust, 12+((int)(realtime*4)&1));

	if (setup_cursor == 0)
		M_DrawCharacter (168 + 8*strlen(setup_hostname), setup_cursor_table [setup_cursor]-VertAdjust, 10+((int)(realtime*4)&1));

	if (setup_cursor == 1)
		M_DrawCharacter (168 + 8*strlen(setup_myname), setup_cursor_table [setup_cursor]-VertAdjust, 10+((int)(realtime*4)&1));
	// jkrige - multiplayer setup menu alignment
}
...
enum 
{
	...
	OPT_LOOKSPRING,	//11
	OPT_LOOKSTRAFE,	//12
	//OPT_CROSSHAIR,	//13 // jkrige - removed crosshair slider
	OPT_ALWAYSMLOOK,//13 jkrige - mlook cvar
	OPT_ALWAYSSLOOK,//14 jkrige - slook cvar
	OPT_SCALE2D, //15 jkrige - scale2d cvar
	OPT_VIDEO,		//16
	OPT_USEMOUSE,	//17
	OPTIONS_ITEMS
};
...
void M_AdjustSliders (int dir)
{
...
#ifdef GLQUAKE
		case OPT_TEXTUREMODE:	// texture mode
	...
	case OPT_MUSICTYPE: // bgm type
		if (strcmpi(bgmtype.string,"cd") == 0)
		{
			if (dir < 0)
				...
				Cvar_Set("bgmtype","mod");
		}
		else if (strcmpi(bgmtype.string,"mod") == 0)
		{
			if (dir < 0)
				Cvar_Set("bgmtype","cd");
			else
		...
		else if (strcmpi(bgmtype.string,"midi") == 0)
		{
			if (dir < 0)
				Cvar_Set("bgmtype","mod");
			else
	...
	// jkrige - removed crosshair slider
	//case OPT_CROSSHAIR:	
	...
	case OPT_ALWAYSSLOOK:
		Cvar_SetValue ("cl_slook", !cl_slook.value); // jkrige - slook cvar
		break;
	case OPT_SCALE2D:
		// jkrige - scale2d cvar
#ifdef GLQUAKE
		if(modelist[(int)vid_mode.value].width >= BASEWIDTH && modelist[(int)vid_mode.value].height >= BASEHEIGHT)
		{
			Cvar_SetValue ("vid_scale2d", !vid_scale2d.value);
			param_scale2d_override = true;
		}
		// jkrige - scale2d cvar
#endif
		break;
...
void M_Options_Draw (void)
{
	...
	M_Print (16, 60+(1*8), "     Reset to defaults");

	M_Print (16, 60+(2*8), "           Screen size");
	// jkrige - viewsize decreased
	//r = (scr_viewsize.value - 30) / (120 - 30);
	r = (scr_viewsize.value - 20) / (110 - 20);
	// jkrige - viewsize decreased
	M_DrawSlider (220, 60+(2*8), r);

	// jkrige - change brightness slider for opengl
	M_Print (16, 60+(3*8), "            Brightness");
#ifdef GLQUAKE
	...
	// jkrige - anisotropic filtering
	M_Print (16, 60+(4*8), "          Texture Mode");
#ifdef GLQUAKE
	if (gl_texturemode.value == 0.0f)
		M_Print (220, 60+(4*8), "Point");
	else if (gl_texturemode.value == 1.0f)
		M_Print (220, 60+(4*8), "Bilinear");
	else if (gl_texturemode.value == 2.0f)
		M_Print (220, 60+(4*8), "Trilinear");
	else
	{
		if(vid_scale2d.value == 0)
			M_Print (220, 60+(4*8), va("Anisotropic %.0fx", pow(2.0f, gl_texturemode.value - 2.0f)));
		else
			M_Print (220, 60+(4*8), va("AF %.0fx", pow(2.0f, gl_texturemode.value - 2.0f)));
	}
#else
	M_Print (220, 60+(4*8), "Point");
#endif
	...
	//M_Print (16, 60+(OPT_CROSSHAIR*8),	"        Show Crosshair"); // jkrige - removed crosshair slider
	//M_DrawCheckbox (220, 60+(OPT_CROSSHAIR*8), crosshair.value); // jkrige - removed crosshair slider

	M_Print (16,60+(OPT_ALWAYSMLOOK*8),	"            Mouse Look");
	M_DrawCheckbox (220, 60+(OPT_ALWAYSMLOOK*8), cl_mlook.value /*in_mlook.state & 1*/); // jkrige - mlook cvar

	M_Print (16,60+(OPT_ALWAYSSLOOK*8),	"            Slope Look");
	M_DrawCheckbox (220, 60+(OPT_ALWAYSSLOOK*8), cl_slook.value); // jkrige - slook cvar

	M_Print (16,60+(OPT_SCALE2D*8),	"              Scale 2D");
	M_DrawCheckbox (220, 60+(OPT_SCALE2D*8), vid_scale2d.value); // jkrige - scale2d cvar
...
//#endif
		...
		// jkrige - anisotropic filtering
#ifndef GLQUAKE
		if ((options_cursor == OPT_TEXTUREMODE)) options_cursor--;
#endif
		// jkrige - anisotropic filtering
...
void M_Quit_Draw (void)
{
	...
	if (LinePos > MaxLines + QUIT_SIZE + 2)
	{
		...
		if (LineTimes >= 2)
		{
			...
			FMOD_MusicStart ("track12", 12, false, false, false);
			// jkrige - fmod sound system (music)
		}
	}
	...
	// jkrige - hexen 2 credits
	//M_PrintWhite (16, y,  "        Hexen II version 1.12       ");	y += 8;
	//M_PrintWhite (16, y,  "         by Raven Software          ");	y += 16;

	//M_PrintWhite (16, y,  "     Hexen II by Raven Software     ");	y += 8;
	//M_PrintWhite (16, y,  va("  Hexen II v%.2f  by Jacques Krige  ", HEXEN2_VERSION));	y += 16;
	M_PrintWhite (16, y,  va("  Hexen II v%.2f  by Jacques Krige  ", HEXEN2_VERSION));	y += 8;
	M_PrintWhite (16, y,  "     http://www.corvinstein.com     ");	y += 16;
	// jkrige - hexen 2 credits
