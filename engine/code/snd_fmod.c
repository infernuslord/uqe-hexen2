...
qboolean SND_FOpen (const char *name, qboolean midi, qboolean resume)
{
	...
	if(midi == true)
	{
		...
		len = COM_FOpenFile ((char *)filefull, &f, false);
	}
	else
	{
...
// ===================================================================================
//
//  MIDI AUDIO CONTROL ROUTINES
//
// ===================================================================================

void Midi_Start (char *name, qboolean loop, qboolean notify, qboolean resume)
{
#ifdef UQE_FMOD_MIDI
	MOD_Start(name, true, loop, notify, resume);
#else
	MIDI_Play(name);
	MIDI_Loop(loop);
#endif
...
void FMOD_MusicStart (char *name, byte track, qboolean loop, qboolean notify, qboolean resume)
{
	...
	if (strcmpi(bgmtype.string, "midi") == 0)
		Midi_Start(name, loop, notify, resume);
