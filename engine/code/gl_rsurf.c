	...
	case GL_RGBA:
		...
		for (i=0 ; i<tmax ; i++, dest += stride)
		{
			for (j=0 ; j<smax ; j++)
			{
				...
				/*t = *bl++;
				t >>= 7;
				//t >>= 8; // jkrige - lightmap brightness
				if (t > 255)
					t = 255;
...
void R_DrawLumaSurfaces (msurface_t *s, int num_surfaces)
{
	...
	for (fa = s, i = 0; i < num_surfaces; fa++, i++)
    {
		...
		if(fa->luma_mark == true && t->tex_luma == true)
        {
			...
			// draw luma textures more than once (hacky?)
			DrawGLPoly (fa->polys);
...
void R_RenderBrushPoly (msurface_t *fa, qboolean override)
{
	...
	if ((currententity->drawflags & MLS_ABSLIGHT) == MLS_ABSLIGHT)
	{
		...
		// rjr
		glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
		// jkrige - bmodels too dark
		if(gl_normalmap_render.value == 1)
			abslight_increase = 120;
		else
			abslight_increase = 72;

		if(currententity->abslight + abslight_increase <= 255)
			intensity = ( float )(currententity->abslight + abslight_increase) / 255.0f;
		else
			intensity = ( float )currententity->abslight / 255.0f;
		// jkrige - bmodels too dark
//		intensity = 0;
	...
	if (gl_normalmap_render.value == 1 && t->tex_norm == true && !(fa->flags & SURF_DRAWTURB) && !(fa->flags & SURF_DRAWSKY) && !(fa->flags & SURF_UNDERWATER))
	{
		glDepthMask (GL_FALSE);
		glEnable (GL_BLEND);
		...
		GL_Bind (JK_NORM_TEX + t->gl_texturenum);
		DrawGLPoly (fa->polys);
...
// instanced model
	...
	if (!Translucent && (currententity->drawflags & MLS_ABSLIGHT) != MLS_ABSLIGHT)
...
//#ifdef QUAKE2 // jkrige - skybox
	//R_ClearSkyBox (); // jkrige - skybox (clipping)
//#endif
