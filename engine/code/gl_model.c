...
void Mod_LoadTextures (lump_t *l)
{
	...
	for (i=0 ; i<m->nummiptex ; i++)
	{
		...
		{
			// jkrige - normal mapping & luma textures
			tx->tex_norm = false;
			tx->tex_luma = false;
			// jkrige - normal mapping & luma textures
			...
			{
				tx->gl_texturenum = GL_LoadTexture (mt->name, "texture", tx->width, tx->height, (byte *)(tx+1), true, false, 0, 1);
			}
			else
			{
				// jkrige - normal mapping & luma textures
				for(j = 0; j < numgltextures; j++)
				{
					...
					{
						tx->tex_norm = gltextures[j].tex_norm;
...
static void Mod_LoadLighting (lump_t *l)
{
	...
	if (gl_lightmap_format == GL_RGBA)
	{
			...
			if (data)
			{
				if (data[0] == 'Q' && data[1] == 'L' && data[2] == 'I' && data[3] == 'T')
				{
					...
					if (i == 1)
					{
						...
						//strcat(con_modload, va("Mod_LoadLighting: Loaded colored light. (32-bit)\n"));
						...
						else
						{
							...
							if (!l->filelen)
							{
								...
								//strcat(con_modload, va("Mod_LoadLighting: No white light data... Using colored only.\n"));
								return;
							}
							Con_DPrintf("Mod_LoadLighting: Loaded white light.\n");
							//strcat(con_modload, va("Mod_LoadLighting: Loaded white light.\n"));
							...
							//strcat(con_modload, va("Mod_LoadLighting: Blended lightmaps.\n"));
					...
					else
					{
						...
						//strcat(con_modload, va("Mod_LoadLighting: Unknown .lit file version (%d)\n", i));
					}
				}
				else
				{
					Con_DPrintf("Mod_LoadLighting: Corrupt .lit file (old version?), ignoring\n");
					//strcat(con_modload, va("Mod_LoadLighting: Corrupt .lit file (old version?), ignoring.\n"));
				}
			}
		}
		...
		//strcat(con_modload, va("Mod_LoadLighting: Loaded white light (32-bit)\n"));
	}
	else
	{
		if (!l->filelen)
		...
		//strcat(con_modload, va("Mod_LoadLighting: Loaded white light (8-bit)\n"));
	}
}
...
// load into heap
	...
	// jkrige - gets "model *1 not found" error when printf anywhere above "Mod_LoadSubmodels" fuckin' strange?...
	Con_Printf("%s", con_modload);
	strset(con_modload, 0);
	// jkrige - gets "model *1 not found" error when printf anywhere above "Mod_LoadSubmodels" fuckin' strange?...
...
void *Mod_LoadAllSkins (int numskins, daliasskintype_t *pskintype, int mdl_flags)
{
	...
	for (i=0 ; i<numskins ; i++)
	{
		...
		sprintf (namefullbright, "fullbright_%s_%i", loadmodel->name, i); // jkrige - fullbright pixels
		...
		pheader->gl_texturenum[i] = LoadTextureImage (model2, "skin", 0, 0, false, true);
		if (pheader->gl_texturenum[i] == 0)// did not find a matching external texture...
		{
			pheader->gl_texturenum[i] = GL_LoadTexture (name, "skin", pheader->skinwidth, pheader->skinheight, (byte *)(pskintype + 1), true, false, texture_mode, 1);
		}
		// jkrige - external texture loading
...
void * Mod_LoadSpriteFrame (model_t *mod, void * pin, mspriteframe_t **ppframe, int framenum)
{
	...
	if(FileFound == -1)
	{
		...
		FileFound = COM_FOpenFile(sprite2, &f2, false);
	}
...
/*	if ((mod->flags & EF_TRANSPARENT))
	...
	if (pspriteframe->gl_texturenum == 0) // did not find a matching external files...
	{
	    ...
	    //    pspriteframe->gl_texturenum = GL_LoadTransTexture (name, width, height, (byte *)(pinframe + 1), true, 127);
	    //else
			pspriteframe->gl_texturenum = GL_LoadTexture (name, "sprite", width, height, (byte *)(pinframe + 1), true, true, 0, 1);
	}
	// jkrige - external texture loading
