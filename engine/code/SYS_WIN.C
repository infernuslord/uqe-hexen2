...
#ifdef GLQUAKE
	//#define MINIMUM_WIN_MEMORY		0x1000000 // jkrige - memory increase: 16 777 216 bytes (16MB)
	//#define MAXIMUM_WIN_MEMORY		0x1800000 // jkrige - memory increase: 25 165 824 bytes (24MB)
	#define MINIMUM_WIN_MEMORY		0x2000000 // jkrige - memory increase: 33 554 432 bytes (32MB)
	#define MAXIMUM_WIN_MEMORY		0x4000000 // jkrige - memory increase: 67 108 864 bytes (64MB)
#else
	//#define MINIMUM_WIN_MEMORY		0x0C00000 // jkrige - memory increase: 12 582 912 bytes (12MB)
	//#define MAXIMUM_WIN_MEMORY		0x1600000 // jkrige - memory increase: 23 068 672 bytes (22MB)
	#define MINIMUM_WIN_MEMORY		0x1000000 // jkrige - memory increase: 16 777 216 bytes (16MB)
	#define MAXIMUM_WIN_MEMORY		0x2000000 // jkrige - memory increase: 33 554 432 bytes (32MB)
#endif
...
void Sys_FileClose (int handle)
{
	...
	if(LoadFromPK3 == true)
	{
		unzCloseCurrentFile(sys_zhandles[handle]);
		sys_zhandles[handle] = NULL;
		LoadFromPK3 = false;
	}
	else
	{
...
// jkrige - pk3 file support
/*long Sys_FileTell (int handle)
{
	int		t;
	long	_tell;

	t = VID_ForceUnlockedAndReturnState ();
	_tell = ftell(sys_handles[handle]);
	VID_ForceLockState (t);

	return _tell;
}*/

int Sys_FFileRead (FILE *f, void *dest, int count)
{
	int		t, x;

	t = VID_ForceUnlockedAndReturnState ();
	x = fread (dest, 1, count, f);
	VID_ForceLockState (t);
	return x;
}
void Sys_FFileClose (FILE *f)
{
	int		t;
...
int Sys_FileRead (int handle, void *dest, int count)
{
	...
	t = VID_ForceUnlockedAndReturnState ();
	if(LoadFromPK3 == true)
		x = unzReadCurrentFile(sys_zhandles[handle], dest, count);
	else
		x = fread (dest, 1, count, sys_handles[handle]);
	VID_ForceLockState (t);
