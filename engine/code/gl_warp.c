...
void EmitBothSkyLayers (msurface_t *fa)
{
	...
	glEnable (GL_BLEND);
	// jkrige - translucent skies
	//glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
    //glColor4f(1.0f, 1.0f, 1.0f, 0.55f /*r_skyalpha.value*/);
	// jkrige - translucent skies
...
void R_LoadSkys (void)
{
	...
	for (i=0 ; i<6 ; i++)
	{
		...
		if(image_bits == 8)
			GL_Upload8 (data, image_width, image_height, true, false, false, 0);
		if(image_bits == 32)
...
void R_DrawSkyChain (msurface_t *s)
{
	...
	/*else // jkrige - skybox (clipping)
	{
		if (gl_skytype.value == 1) // jkrige - skyshader
		{
			c_sky = 0;
			GL_Bind(solidskytexture);
...
void R_InitSky (texture_t *mt)
{
	...
	if(FoundSolid == -1)
	{
		...
		FoundSolid = COM_FOpenFile(skytex_solid, &f2, false);
	}
	if(FoundSolid == -1)
	{
	...
	if(FoundAlpha == -1)
	{
		...
		FoundAlpha = COM_FOpenFile(skytex_alpha, &f2, false);
	}
	if(FoundAlpha == -1)
	{
