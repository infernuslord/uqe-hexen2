...
// and are immediately discarded by quake
		...
		else if (strcmpi(keyname,"MOD") == 0) // jkrige - fmod sound system (music)
		{
			strcpy(sv.mod_name,com_token);
			continue;
		}
		else if (strcmpi(keyname,"SKY") == 0) // jkrige - skybox
		{
			strcpy(sv.skybox, com_token);
			continue;
		}
