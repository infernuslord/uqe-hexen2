...
// jkrige - particle texture
#if GLQUAKE
cvar_t		r_particle_dot = {"r_particle_dot","1", true};
#endif
// jkrige - particle texture
...
void R_InitParticles (void)
{
	...
	// jkrige - particle texture
#if GLQUAKE
	Cvar_RegisterVariable (&r_particle_dot);
#endif
	// jkrige - particle texture
	...
	len = COM_FOpenFile ("gfx/tinttab.lmp", &f, false);
	if(len < 1)
		Sys_Error ("Couldn't open gfx/tinttab.lmp");

	transTable = COM_FReadFile(f, len);
	if (!transTable)
		Sys_Error ("Couldn't load gfx/tinttab.lmp");
	// jkrige - pk3 file support
}	
...
//					break;
		...
		else
		{
			...
			// jkrige - particle texture
			if(!r_particle_dot.value)
			{
				glTexCoord2f (1,0);
				glVertex3fv (p->org);
				glTexCoord2f (1,0.60); // jkrige - was 1, 0.5
				glVertex3f (p->org[0] + r_pup[0]*scale, p->org[1] + r_pup[1]*scale, p->org[2] + r_pup[2]*scale);
				glTexCoord2f (0.40,0); // jkrige - was 0.5, 0
				glVertex3f (p->org[0] + r_pright[0]*scale, p->org[1] + r_pright[1]*scale, p->org[2] + r_pright[2]*scale);
			}
			else
			{
