...
// jkrige - portals game
qboolean	hexen2_standard = true;
qboolean	hexen2_portals = false;
// jkrige - portals game

// jkrige - configurable mission pack
qboolean	hexen2_progs = false;
// jkrige - configurable mission pack
...
void COM_SetScale2D()
{
	...
	AspectRatio = (float)modelist[(int)vid_mode.value].height / (float)modelist[(int)vid_mode.value].width;
	Scale2DFactor = ceil((float)modelist[(int)vid_mode.value].width / (float)BASEWIDTH);

	if(Scale2DFactor < 2.0f)
		Scale2DFactor = 2.0f;

	//Scale2DHeight = (float)modelist[(int)vid_mode.value].height / 2.0f;
	//Scale2DWidth = (float)Scale2DHeight / AspectRatio;

	Scale2DWidth = (float)modelist[(int)vid_mode.value].width / Scale2DFactor;
	Scale2DHeight = (float)AspectRatio * Scale2DWidth;
}
// jkrige - scale2d
...
// reconstitute the command line for the cmdline externally visible cvar
	...
	// jkrige - portals game
	if (COM_CheckParm ("-portals") | COM_CheckParm ("-h2mp") | COM_CheckParm ("-pop"))
	{
		hexen2_standard = false;
		hexen2_portals = true;
	}
	// jkrige - portals game
	...
	// jkrige - configurable mission pack
	if (COM_CheckParm ("-standardprogs"))
	{
		hexen2_progs = true;
	}
	// jkrige - configurable mission pack
...
{
	// jkrige - pk3 file support (pack / zip)
	int		ident;
	unzFile	uzf;
	// jkrige - pk3 file support (pack / zip)
...
// parse the directory
	...
	pack->ident = IDPAKHEADER; // jkrige - pk3 file support
...
//
	...
	if (handle != -1)
	{
		do
		{
			...
			// jkrige - remove oem
			//if (!strcmp(fileinfo.name, "pak1.pak"))
			//	Cvar_Set ("oem", "0");
			// jkrige - remove oem
	...
	if (handle != -1)
	{
		do
		{
			...
			// jkrige - remove oem
			//if (!strcmp(fileinfo.name, "pak1.pk3"))
			//	Cvar_Set ("oem", "0");
			// jkrige - remove oem
...
//
	...
	// jkrige - remove oem
	//Cvar_Set ("oem", "1"); // jkrige - oem issue
	// jkrige - remove oem

	//COM_AddGameDirectory (va("%s/"GAMENAME, basedir) );
	COM_AddGameDirectory (va("%s/%s", basedir, GAMENAME) ); // jkrige - data1

	// jkrige - portals game + configurable mission pack
	//COM_AddGameDirectory (va("%s/portals", basedir) );
	if(hexen2_portals == true | hexen2_progs == false) // if (COM_CheckParm ("-portals") | COM_CheckParm ("-h2mp") | COM_CheckParm ("-pop"))
		COM_AddGameDirectory (va("%s/portals", basedir) );
	// jkrige - portals game + configurable mission pack
