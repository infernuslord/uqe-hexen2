...
cvar_t		gl_normalmap_generate = {"gl_normalmap_generate", "0", true};
cvar_t		gl_normalmap_render = {"gl_normalmap_render", "1", true};
// jkrige - normal mapping
...
void Draw_TextureMode_f (void)
{
	...
	for (j=0, glt=gltextures ; j<numgltextures ; j++, glt++)
	{
		if (glt->mipmap)
		{
			...
			// jkrige - normal mapping
			if(glt->tex_norm == true)
			{
				GL_Bind (JK_NORM_TEX + glt->texnum);
...
void Draw_Init (void)
{
	...
	// jkrige - normal mapping
	Cvar_RegisterVariable (&gl_normalmap_generate);
	Cvar_RegisterVariable (&gl_normalmap_render);
	// jkrige - normal mapping
	...
	Cvar_RegisterVariable (&gl_framebuffer);
	Cvar_RegisterVariable (&gl_framebuffer_msaa);
	// jkrige - framebuffer object
...
void GL_Upload32 (unsigned *data, int width, int height, qboolean mipmap, qboolean alpha)
{
	...
	static	unsigned	scaled[2048*1024];	// [1024*512]; // jkrige - doubled image load size
...
typedef union _pixelint 
{ 
   pixel rgba; 
   unsigned int ui; 
} pixelint; 

void WritePixel (unsigned int *image, unsigned int *pix, int x, int y, int w) 
{ 
   *(image + w * y + x) = *pix; 
}

void ReadPixel (unsigned int *image, pixelint *pix, int x, int y, int w) 
{ 
   int gs; 

   pix->ui = *(image + w * y + x); 

   // write greyscale to alpha - NTSC standard 
   // the ATI sample code i based this on only used the red channel, which gave wild results on 
   // some texes.  this is much more balanced. 
   // I'm storing in alpha so that the rgb info is not interfered with, as the generator will 
   // need it again later on. 

   gs = ((int) pix->rgba.red) * 30; 
   gs += ((int) pix->rgba.green) * 59; 
   gs += ((int) pix->rgba.blue) * 11; 
   pix->rgba.alpha = (byte) (gs / 100);
} 
...
void MakeDOT3 (gltexture_t *glt, unsigned int *src, int width, int height, qboolean alpha, int mode, int bytesperpixel) 
{ 
   unsigned int *dst = (unsigned int *) malloc (width * height * sizeof (unsigned int)); 
   int x; 
   int y; 
   pixelint pix; 
   ...
   if(bytesperpixel == 1)
   {
	   GL_Upload8 ((void *)src, width, height, false, true, alpha, mode);
		...
		{ 
			...
			{ 
				// Do Y Sobel filter 
				ReadPixel (trans, &pix, (x - 1 + width) % width, (y + 1) % height, width); 
				dY  = ((float) pix.rgba.alpha) / 255.0f * -1.0f; 

				ReadPixel(trans, &pix, x % width, (y + 1) % height, width); 
				dY += ((float) pix.rgba.alpha) / 255.0f * -2.0f; 

				ReadPixel(trans, &pix, (x + 1) % width, (y + 1) % height, width); 
				dY += ((float) pix.rgba.alpha) / 255.0f * -1.0f; 

				ReadPixel(trans, &pix, (x - 1 + width) % width, (y - 1 + height) % height, width); 
				dY += ((float) pix.rgba.alpha) / 255.0f *  1.0f; 

				ReadPixel(trans, &pix, x % width, (y - 1 + height) % height, width); 
				dY += ((float) pix.rgba.alpha) / 255.0f *  2.0f; 

				ReadPixel(trans, &pix, (x + 1) % width, (y - 1 + height) % height, width); 
				dY += ((float) pix.rgba.alpha) / 255.0f *  1.0f; 
				...
				ReadPixel(trans, &pix, (x - 1 + width) % width, y % height, width); 
				...
				ReadPixel(trans, &pix, (x + 1) % width, y % height, width); 
				dX += ((float) pix.rgba.alpha) / 255.0f *  2.0f; 
				...
				WritePixel (dst, &pix.ui, x, y, width); 
			} 
		}
   ...
   else
   {
	...
	{ 
		...
		{ 
			// Do Y Sobel filter 
			ReadPixel (src, &pix, (x - 1 + width) % width, (y + 1) % height, width); 
			dY  = ((float) pix.rgba.alpha) / 255.0f * -1.0f; 

			ReadPixel(src, &pix, x % width, (y + 1) % height, width); 
			dY += ((float) pix.rgba.alpha) / 255.0f * -2.0f; 

			ReadPixel(src, &pix, (x + 1) % width, (y + 1) % height, width); 
			dY += ((float) pix.rgba.alpha) / 255.0f * -1.0f; 

			ReadPixel(src, &pix, (x - 1 + width) % width, (y - 1 + height) % height, width); 
			dY += ((float) pix.rgba.alpha) / 255.0f *  1.0f; 

			ReadPixel(src, &pix, x % width, (y - 1 + height) % height, width); 
			dY += ((float) pix.rgba.alpha) / 255.0f *  2.0f; 

			ReadPixel(src, &pix, (x + 1) % width, (y - 1 + height) % height, width); 
			dY += ((float) pix.rgba.alpha) / 255.0f *  1.0f; 
			...
			ReadPixel(src, &pix, (x - 1 + width) % width, y % height, width); 
			dX += ((float) pix.rgba.alpha) / 255.0f * -2.0f; 
			...
			ReadPixel(src, &pix, (x + 1) % width, y % height, width); 
			dX += ((float) pix.rgba.alpha) / 255.0f *  2.0f; 
   ...
   GL_Upload32(dst, width, height, true, false);
...
int GL_LoadTexture (char *identifier, char *textype, int width, int height, byte *data, qboolean mipmap, qboolean alpha, int mode, int bytesperpixel)
{
	...
	// jkrige - reset normal & luma textures
	glt->tex_norm = false;
	glt->tex_luma = false;
	// jkrige - reset normal & luma textures
	...
	if (!isDedicated)
	{
		...
		if (!strcmp (textype, "texture"))
		{
			...
			if (ch_dot != NULL)
			{
				...
				strcat(ch_name, "_norm");
				strcat(ch_name, ch_dot);
				identifier = ident;

				if (!(normdata = LoadImagePixels (ch_name, false)))
				{
					if(gl_normalmap_generate.value == 1)
					{
						GL_Bind(JK_NORM_TEX + glt->texnum);
						MakeDOT3(glt, (void *)data, width, height, alpha, mode, bytesperpixel);
				...
				{
					GL_Bind(JK_NORM_TEX + glt->texnum);
					GL_Upload32((unsigned int *)normdata, width, height, mipmap, false);
					free(normdata);
