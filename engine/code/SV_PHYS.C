...
{
	int		i;
	float	wishspeed; // jkrige - SV_MAXVELOCITY fix
...
void SV_PushMove (edict_t *pusher, float movetime, qboolean update_time)
{
	...
	float	solid_backup; // jkrige - MOVETYPE_PUSH fix
...
void SV_Physics_Pusher (edict_t *ent)
{
	...
	if (movetime)
	{
//#ifdef QUAKE2RJ // jkrige - portals game
...
//
	...
	for (i=0 ; i<sv.num_edicts ; i++, ent = NEXT_EDICT(ent))
	{
		...
		if (PR_GLOBAL_STRUCT(force_retouch)) // jkrige - portals game
		{
