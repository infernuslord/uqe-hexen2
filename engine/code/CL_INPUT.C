...
//void IN_KLookDown (void) {KeyDown(&in_klook);} // jkrige - removed klook command
//void IN_KLookUp (void) {KeyUp(&in_klook);} // jkrige - removed klook command
// jkrige - mlook cvar
...
void CL_AdjustAngles (void)
{
	...
	// jkrige - ring of flight fix
	//if (up || down)
	//	V_StopPitchDrift ();

	if ((up || down) | (cl.v.movetype == MOVETYPE_FLY))
		V_StopPitchDrift ();
	// jkrige - ring of flight fix
...
//	cmd->sidemove -= cl_sidespeed.value * CL_KeyState (&in_moveleft);
	...
	cmd->forwardmove += 200 * CL_KeyState (&in_forward); // jkrige - ??
		//cmd->forwardmove -= cl_backspeed.value * CL_KeyState (&in_back);
	cmd->forwardmove -= 200 * CL_KeyState (&in_back); // jkrige - ??
	//}	
	// jkrige - removed klook command
...
//
	...
	if ( ( (cl_forwardspeed.value <= 200 && in_speed.state & 1) || (cl_forwardspeed.value > 200 && !(in_speed.state & 1)) ) && (cl.v.hasted <= 1) )
	{
...
void CL_ClearStates (void)
{
	...
	//in_mlook.state = 0; // jkrige - mlook cvar
	//in_klook.state = 0; // jkrige - removed klook command
	in_left.state = 0;
	in_right.state = 0;
	in_forward.state = 0;
	in_back.state = 0;
