...
void SB_Draw(void)
{
	...
	int maxMana;
	int y;
	//static float scr_viewsize_tmp;

	// jkrige - sbar height
	/*if(scr_viewsize_tmp != scr_viewsize.value)
	{
...
static void DrawLowerBar(void)
{
	...
	//}
	/*if(playerClass < 1 || playerClass > NUM_CLASSES-1) // jkrige - commented (?)
	{ // Default to assasin                              // jkrige - commented (?)
		playerClass = NUM_CLASSES-2;                     // jkrige - commented (?)
	}*/
	// jkrige - old mission cvar
	...
	{
		if(playerClass < 1 || playerClass > NUM_CLASSES-1)
		{ // Default to assasin
			//playerClass = NUM_CLASSES-1;
			// jkrige - portals game
			playerClass = 1;	// Default to paladin
...
static void ShowInfoDown_f(void)
{
	if(!sv.active) // jkrige - fixed statusbar activity on menu and demo playback
		return;
...
static void ShowInfoUp_f(void)
{
	...
	/* 100 = brown bottom stripe w/status bar
       110 = full screen w/status bar (was malfunctioning)
       120 = full screen w/no status bar */
	if(cl.intermission || scr_viewsize.value >= 110.0)
	{
		BarTargetHeight = 0.0-BAR_BUMP_HEIGHT;
	}
